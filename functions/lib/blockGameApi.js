...
exports.handleBlockGameApi = handleBlockGameApi;
...
function serializePinDoc(pinId, data) {
...
async function handleBlockGameApi(req, res, path, deps) {
    ...
    if (req.method === "POST" && path === "/block-game/sim-pin/generate") {
        ...
        const recipientNote = recipientName && recipientEmail
    ...
    if (req.method === "POST" && path === "/block-game/settings/bomb-ranges") {
        ...
        for (const [key, val] of Object.entries(raw)) {
            ...
            if (!Number.isFinite(pctMin))
                pctMin = 0.3;
            if (!Number.isFinite(pctMax))
                pctMax = 0.55;
            if (pctMin > pctMax)
                [pctMin, pctMax] = [pctMax, pctMin];
        ...
        try {
            const ok = await db.runTransaction(async (tx) => {
                ...
                if (status === "approved") {
                    ...
                    prevWallet = wSnap.exists ? wSnap.data() : null;
                }
                tx.set(reqRef, {
                    status,
