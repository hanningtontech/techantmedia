...
const STAKE = 10;
const SIM_ROUNDS = 5;
function totalCells(rows, cols) {
    return rows * cols;
}
function consecutiveWinProbability(total, bombs, rounds) {
    if (rounds <= 0)
        return 1;
    const safe = total - bombs;
    if (rounds > safe || total <= 0)
        return 0;
    let p = 1;
...
function calculateMultiplier(houseEdge, round, total, bombs) {
    const pWin = consecutiveWinProbability(total, bombs, round);
    if (pWin <= 0)
        return 0;
    return (1 - houseEdge) / pWin;
}
function simulateOneGame(houseEdge, rng = Math.random) {
...
function createTick(prev, stake, payout, gameIndex, t) {
...
async function pumpLiveChartOnce() {
