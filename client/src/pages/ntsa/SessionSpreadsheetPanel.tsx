...
import { DOCUMENT_TYPES } from "@shared/documentExtraction";
...
import { EditableSpreadsheetCell } from "@/pages/ntsa/EditableSpreadsheetCell";
...
type Props = {
  ...
  onRemoveRow: (globalIndex: number) => void;
  onUpdateCell?: (globalIndex: number, fieldId: ExtractionFieldId, value: string) => void;
};
...
export function SessionSpreadsheetPanel({ rows, meta, onRemoveRow, onUpdateCell }: Props) {
  const [page, setPage] = useState(1);
  const totalPages = Math.max(1, Math.ceil(rows.length / ROWS_PER_PAGE));
  const columns = getSpreadsheetColumns(meta);
  const totalKes = sessionTotalKes(rows, meta);
  const docLabel = DOCUMENT_TYPES[meta.documentType].label;
    ...
    <div className="w-full overflow-hidden rounded-2xl border border-white/10 bg-[#12121a]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
        <div className="flex items-center gap-2 text-white">
          ...
          <div>
            <h2 className="font-semibold">Session spreadsheet</h2>
            <p className="text-xs text-zinc-500">
              {docLabel}
              {onUpdateCell ? " · Click any cell to edit" : ""}
            </p>
          </div>
        </div>
      ...
      {rows.length === 0 ? (
        <div className="px-5 py-16 text-center text-sm text-zinc-500">
          No documents extracted yet. Upload an image or PDF to begin.
        </div>
      ) : (
        <>
          <PaginationBar
            ...
            <table className="w-full table-fixed text-left text-sm">
              ...
              <thead>
                <tr className="border-b border-white/10 bg-[#1f6b6b]">
                  ...
                  <th className="w-10 px-1 py-2.5" aria-label="Actions" />
                </tr>
              </thead>
              <tbody>
                      ...
                      {columns.map((col) => (
                        ...
                        >
                          <EditableSpreadsheetCell
                            fieldId={col.fieldId}
                            value={getSpreadsheetCellValue(row, col.fieldId)}
                            align={col.align}
                            editable={Boolean(onUpdateCell)}
                            onCommit={(value) => onUpdateCell?.(globalIndex, col.fieldId, value)}
                          />
                        </td>
                      ))}
