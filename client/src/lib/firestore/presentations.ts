...
function mapDoc(id: string, d: DocumentData): ClassPresentation {
  return {
    id,
...
export async function listPublishedPresentations(opts?: { className?: string; limit?: number }) {
  const db = tryGetFirestoreDb();
  if (!db) return [] as ClassPresentation[];
...
export async function listAllPresentations(opts?: { limit?: number }) {
  const db = tryGetFirestoreDb();
  if (!db) return [] as ClassPresentation[];
...
export async function getPresentationById(id: string): Promise<ClassPresentation | null> {
  const db = tryGetFirestoreDb();
  if (!db) return null;
  const snap = await getDoc(doc(db, COL, id));
  if (!snap.exists()) return null;
  return mapDoc(snap.id, snap.data() as DocumentData);
}

export async function uploadClassPresentation(args: {
  file: File;
  title: string;
  description?: string;
  className?: string;
  ...
  const name = f.name || "presentation.pptx";
  const contentType = f.type || "application/vnd.openxmlformats-officedocument.presentationml.presentation";
  if (!name.toLowerCase().endsWith(".pptx")) throw new Error("Only .pptx files are supported");
  ...
  const resp = await fetch(`/api/b2/presentations/${encodeURIComponent(ref.id)}`, {
    method: "POST",
    headers: {
...
export async function setPresentationPublished(id: string, published: boolean) {
  const db = tryGetFirestoreDb();
  if (!db) throw new Error("Firebase is not configured");
  await updateDoc(doc(db, COL, id), { published, updatedAt: serverTimestamp() });
}

export async function deletePresentation(id: string, b2: { fileId: string; fileName: string }) {
  const db = tryGetFirestoreDb();
  const auth = tryGetFirebaseAuth();
  if (!db || !auth) throw new Error("Firebase is not configured");
  const user = auth.currentUser;
  ...
  const resp = await fetch(`/api/b2/presentations`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${idToken}`,
