...
import type { PortfolioImageUploadProgress } from "@/lib/portfolio/portfolioImages";
...
/** Upload a client gallery image to B2 (admin only). */
export async function uploadClientGalleryImage(
  ...
  const url = `/api/b2/client-gallery-images/${encodeURIComponent(clientUserId)}/${encodeURIComponent(uploadId)}`;
