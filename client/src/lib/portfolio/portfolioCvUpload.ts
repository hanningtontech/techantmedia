...
/** Admin uploads the general developer CV PDF for `/development`. */
export async function uploadPortfolioCv(
  file: File,
  ...
  const name = (file.name || "Hannington_Kuria_Njuguna_Developer_CV.pdf").trim() || "Hannington_Kuria_Njuguna_Developer_CV.pdf";
  ...
  const url = `/api/b2/portfolio-cv/${encodeURIComponent(uploadId)}`;
