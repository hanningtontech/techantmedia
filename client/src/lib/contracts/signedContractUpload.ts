...
/** Client uploads a signed contract (PDF or image) after downloading from the site. */
export async function uploadSignedContractFile(
  file: File,
  onProgress?: (percent: number) => void,
): Promise<{ downloadUrl: string; fileName: string }> {
  const auth = tryGetFirebaseAuth();
  if (!auth?.currentUser) throw new Error("Sign in to upload a signed contract");

  const name = (file.name || "signed-contract.pdf").trim() || "signed-contract.pdf";
  ...
  const url = `/api/b2/signed-contracts/${encodeURIComponent(auth.currentUser.uid)}/${encodeURIComponent(uploadId)}`;
